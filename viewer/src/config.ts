/**
 * Build-time configuration for the Viewer.
 *
 * Vite injects `import.meta.env.VITE_*` variables at build time.
 * Anything NOT prefixed with `VITE_` is unavailable in the browser
 * bundle for safety, so configuration that needs to be visible to
 * the React app MUST be exposed via a `VITE_` env var.
 */
